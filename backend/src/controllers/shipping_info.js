...
export const getShippingInfoById = asyncError(async (req, res, next) => {
	const currentUser = req.user
	const shippingInfoId = req.params.id
	const shippingInfo = await ShippingInfo.findById(shippingInfoId).populate('user')

	if (shippingInfo.user.id !== currentUser._id) {
		next(new ErrorHandler('Shipping info does not belong to current user', 400))
	}
...
export const getAllCities = asyncError(async (req, res, next) => {
