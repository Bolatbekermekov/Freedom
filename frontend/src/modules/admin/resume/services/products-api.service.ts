...
class AdminProductsAPIClass {
	...
	base = `${environment.apiUrl}/api/v2/admin/resume`
	...
	async uploadResume(dto: CreateResumeDTO) {
		...
		dto.files.forEach(file => {
			formData.append('files', file) // Массив файлов, отправляемых на сервер
		})
		return axiosWithAuth.post(`${this.base}/upload-resume`, formData, {
			headers: {
